...
function AssignStartingPlots.Create()
	...
	local findStarts = {
		...
		centerBias = 20, -- % of radius from region center to examine first
...
function AssignStartingPlots:PlaceImpactAndRipples(x, y)
	...
	local ripple_values = {97, 95, 92, 88, 83, 77, 70, 62, 51, 41, 30, 18, 5};
	...
	self:PlaceResourceImpact(x, y, 5, 7) -- Add CS layer with radius of 8 tiles
...
function AssignStartingPlots:ChooseLocations(args)
	...
	for assignIndex = 1, iNumRegions do
		...
		if res_reg[currentRegionNumber] == false and iNumCoastNeeded > 0 then
			...
			NoCoastChance = true;
			chancetobecoast = Map.Rand(100, "");
			print("Chance for coast: " .. chancetobecoast);
			if chancetobecoast > 70 then
				NoCoastChance = false;
			end
			bSuccessFlag, bForcedPlacementFlag = self:FindStart(currentRegionNumber, NoCoastChance)
		end
...
function AssignStartingPlots:AttemptToPlaceBonusResourceAtPlot(x, y, bAllowOasis, Fish_Count)
	...
	elseif plotType == PlotTypes.PLOT_HILLS and featureType == FeatureTypes.NO_FEATURE then
		...
		local terrainType = plot:GetTerrainType();
		if placethis > 75 and terrainType ~= TerrainTypes.TERRAIN_DESERT then
	...
	elseif plotType == PlotTypes.PLOT_OCEAN then
		if terrainType == TerrainTypes.TERRAIN_COAST and featureType == FeatureTypes.NO_FEATURE then
			if plot:IsLake() == false and Fish_Count < 2 then -- Place Fish
...
function AssignStartingPlots:AddStrategicBalanceResources(region_number)
	...
	if self.resource_setting == 8 or self.resource_setting == 9 or self.resource_setting == 4 or self.resource_setting == 5 then
	...
	if self.resource_setting == 7 or self.resource_setting == 9 or self.resource_setting == 4  or self.resource_setting == 5 then
...
function AssignStartingPlots:NormalizeStartLocation(region_number)
	...
	if self.resource_setting == 6 or self.resource_setting == 7 or self.resource_setting == 8 or self.resource_setting == 9 or self.resource_setting == 4 or self.resource_setting == 5 then
	...
	if innerHammerScore <= 4 and earlyHammerScore < 6 then -- Add a small Horse or Iron to second ring.
		if isEvenY then
	...
	if self.resource_setting == 4 or self.resource_setting == 5 then
	...
	if iNumFoodBonusNeeded > 0 then
		...
		while iNumFoodBonusNeeded > 0 do
			if ((innerPlaced < 2 and innerCanHaveBonus > 0) or (self.resource_setting == 4 and innerPlaced < 3 and innerCanHaveBonus > 0) or (self.resource_setting == 5 and innerPlaced < 3 and innerCanHaveBonus > 0))
			...
			elseif ((innerPlaced + outerPlaced < 5 and outerCanHaveBonus > 0) or (self.resource_setting == 4 and innerPlaced + outerPlaced < 4 and outerCanHaveBonus > 0) or (self.resource_setting == 5 and innerPlaced + outerPlaced < 4 and outerCanHaveBonus > 0))
...
function AssignStartingPlots:AssignLuxuryToRegion(region_number)
			...
			if self.iNumTypesAssignedToRegions < self.iNumMaxAllowedForRegions or test == true then -- Not a new type that would exceed number of allowed types, so continue.
				...
				if res_ID == self.whale_ID or res_ID == self.pearls_ID or res_ID == self.crab_ID then
					if self.startLocationConditions[region_number][1] == true then -- This region's start is along an ocean, so water-based luxuries are allowed.
						if self.regionTerrainCounts[region_number][8] >= 3 then -- Enough water available.
		...
		for index, resource_options in ipairs(self.luxury_fallback_weights) do
			...
			if self.luxury_assignment_count[res_ID] < 3 then -- This type still eligible.
				...
				if self.iNumTypesAssignedToRegions < self.iNumMaxAllowedForRegions or test == true then -- Won't exceed allowed types.
					if res_ID == self.whale_ID or res_ID == self.pearls_ID or res_ID == self.crab_ID then
						if self.startLocationConditions[region_number][1] == true then -- This region's start is along an ocean, so water-based luxuries are allowed.
							if self.regionTerrainCounts[region_number][8] >= 8 then -- Enough water available.
								table.insert(resource_IDs, res_ID);
	...
	for index = 1, iNumAvailableTypes do
		...
		if resource_IDs[index] == 13 or resource_IDs[index] == 14 or resource_IDs[index] == 32 then
			coast_lux = true;
...
function AssignStartingPlots:PlaceLuxuries()
	...
	for loop, reg_data in ipairs(self.regions_sorted_by_type) do
		...
		local iNumToPlace = 2;
		if self.resource_setting == 4 or self.resource_setting == 5 then -- Legendary Start
			iNumToPlace = 3;
		end
	...
	if self.resource_setting ~= 1 then
		for region_number = 1, self.iNumCivs do
			...
			if (self.resource_setting ~= 4) and (self.resource_setting ~= 5) and (self.resource_setting ~= 6) then
	...
	for region_number, res_ID in ipairs(self.region_luxury_assignment) do
		...
		elseif self.resource_setting == 3 or self.resource_setting == 4 or self.resource_setting == 5 then
			targetNum = targetNum + 2;
		end
...
function AssignStartingPlots:PlaceSexyBonusAtCivStarts()
	...
	self.deer_ID, self.wheat_ID, self.sheep_ID, self.wheat_ID, self.cow_ID, self.cow_ID, self.wheat_ID};
...
function AssignStartingPlots:GetMajorStrategicResourceQuantityValues()
	...
	elseif self.resource_setting == 3 then -- Abundant
		uran_amt, horse_amt, oil_amt, iron_amt, coal_amt, alum_amt = 2, 6, 9, 9, 10, 10;
	end
...
function AssignStartingPlots:PlaceStrategicAndBonusResources()
	...
	while self.amounts_of_resources_placed[self.uranium_ID + 1] < 8 * self.iNumCivs do
