...
function NQMP_Freedom_T1_Free_Trade_OnPolicyAdopted(playerID, policyID)
	...
	if (player:IsAlive()) then
		if ((player:GetLateGamePolicyTree() == 10) or (player:GetLateGamePolicyTree() == 11)) then
			for loopCity in player:Cities() do
					if (loopCity:GetNumRealBuilding(GameInfoTypes["DUMMY_BUILDING_T1_FREE_TRADE"]) > 0) then
						loopCity:SetNumRealBuilding(GameInfoTypes["DUMMY_BUILDING_T1_FREE_TRADE"], 0);
				end
			end
		end
	end
