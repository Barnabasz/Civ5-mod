...
function NQMP_UpdateENTREPRENEURSHIP(iPlayer)
...
function NQMP_ENTREPRENEURSHIP_PlayerDoTurn(iPlayer)

	local player = Players[iPlayer]
	if (player:IsEverAlive()) then
		if (player:HasPolicy(GameInfo.Policies["POLICY_ENTREPRENEURSHIP"].ID)) then
			NQMP_UpdateENTREPRENEURSHIP(iPlayer)
		end
	end

end
GameEvents.PlayerDoTurn.Add(NQMP_ENTREPRENEURSHIP_PlayerDoTurn);
