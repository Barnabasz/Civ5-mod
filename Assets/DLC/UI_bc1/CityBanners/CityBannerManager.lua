...
local function BannerToolTip( text )
	g_toolTipControls.Text:SetText( text )
...
local g_cityToolTips = {
	...
	CityBannerButton = function( city )
		...
		local tip = city:GetName() --L("TXT_KEY_CITY_OF", cityOwner and cityOwner:GetCivilizationAdjectiveKey() or "", city:GetName() )
	...
	CityBannerRightBackground = function( city )
		...
		if cityOwner then
			...
			if g_activeTeam:IsHasMet( cityTeamID ) then

				if cityOwner:IsMinorCiv() then
					tip = GetCityStateStatusToolTip( g_activePlayerID, cityOwnerID, true )
				else
...
--			tip = GetHelpTextForUnit( unitProductionID, false )

		elseif buildingProductionID ~= -1 then
--			tip = GetHelpTextForBuilding( buildingProductionID, true, true, true, city )

		elseif projectProductionID ~= -1 then
--			tip = GetHelpTextForProject( projectProductionID, false )

		elseif processProductionID ~= -1 then
			tip = GetHelpTextForProcess( processProductionID, false )
			isProducingSomething = false
		else
			if isNoob then
				tip = L( "TXT_KEY_CITY_NOT_PRODUCING", cityName )
			else
		...
		if isProducingSomething then
			...
			tip = "[COLOR_YIELD_FOOD]" .. ToUpper( cityProductionName ) .. "[ENDCOLOR]"
			if isNoob then
				tip = L( "TXT_KEY_PROGRESS_TOWARDS", tip )
			end
			tip = tip .. "  " .. productionStored100 / 100 .. "[ICON_PRODUCTION]/ " .. productionNeeded .. "[ICON_PRODUCTION]"
			if productionPerTurn100 > 0 then
				...
				if productionTurnsLeft > 1 then
					tip = tip .. L( "TXT_KEY_STR_TURNS", productionTurnsLeft -1 )
						.. format( " %+g[ICON_PRODUCTION]  ", ( productionOverflow100 - productionPerTurn100 ) / 100 )
				end
				tip = tip .. "[COLOR_YIELD_FOOD]" .. ToUpper( L( "TXT_KEY_STR_TURNS", productionTurnsLeft ) ) .. "[ENDCOLOR]"
						.. format( " %+g[ICON_PRODUCTION]", productionOverflow100 / 100 )
			end
		end
	...
	-- City Growth ToolTip
	CityBannerLeftBackground = function( city )
		local cityName = city:GetName()
		local cityPopulation = city:GetPopulation()
		local tip = cityName.." "..cityPopulation.."[ICON_CITIZEN]"

		if city:GetTeam() == g_activeTeamID then
			local tipText
			local isNoob = not OptionsManager.IsNoBasicHelp()
			...
			if foodPerTurnTimes100 < 0 then
				...
				tip = tip .. " " .. L"TXT_KEY_CITYVIEW_STARVATION_TEXT"
				tipText = L( "TXT_KEY_PROGRESS_TOWARDS", "[COLOR_WARNING_TEXT]" .. cityPopulation - 1 .. "[ENDCOLOR][ICON_CITIZEN]" )
			elseif city:IsForcedAvoidGrowth() then
				tip = tip .. "[ICON_LOCKED]" .. "[COLOR_YELLOW]" .. L"TXT_KEY_CITYVIEW_FOCUS_AVOID_GROWTH_TEXT" .. "[ENDCOLOR]"
				tipText = "[COLOR_YELLOW]" .. L"TXT_KEY_CITYVIEW_STAGNATION_TEXT" .. "[ENDCOLOR]"
				foodPerTurnTimes100 = 0
			elseif foodPerTurnTimes100 == 0 then
				tipText = "[COLOR_YELLOW]" .. L"TXT_KEY_CITYVIEW_STAGNATION_TEXT" .. "[ENDCOLOR]"
			else
				tipText = L( "TXT_KEY_PROGRESS_TOWARDS", "[COLOR_POSITIVE_TEXT]" .. cityPopulation +1 .. "[ENDCOLOR][ICON_CITIZEN]" )
			end

			tipText = tipText .. "  " .. foodStoredTimes100 / 100 .. "[ICON_FOOD]/ " .. foodThreshold .. "[ICON_FOOD]"
			...
			if foodPerTurnTimes100 ~= 0 then
				...
				if turnsToCityGrowth > 1 then
					tipText = format( "%s[NEWLINE]%s %+g[ICON_FOOD]  ", tipText, L( "TXT_KEY_STR_TURNS", turnsToCityGrowth -1 ), ( foodOverflowTimes100 - foodPerTurnTimes100 ) / 100 )
				else
					tipText = tipText .. "[NEWLINE]"
				end
				tipText = format( "%s%s%s[ENDCOLOR] %+g[ICON_FOOD]", tipText, foodPerTurnTimes100 < 0 and "[COLOR_WARNING_TEXT]" or "[COLOR_POSITIVE_TEXT]", ToUpper( L( "TXT_KEY_STR_TURNS", turnsToCityGrowth ) ), foodOverflowTimes100 / 100 )
			end
			tip = tip .."[NEWLINE]"..tipText
		end
		return BannerToolTip( tip )
	end,
	CityReligion = function( city )
		return BannerToolTip( GetReligionTooltip( city ) )
	end,
	CityFocus = function( city )
		return BannerToolTip( g_cityFocusTooltips[ city:GetFocusType() ] )
	end,
	CityQuests = function( city )
		...
		if cityOwner and cityOwner:IsMinorCiv() then
			tip = GetActiveQuestToolTip( g_activePlayerID, cityOwnerID )
		else
			-- We love the king
			...
			elseif resource then
				...
				for playerID = 0, #Players do
					...
					then
						...
						if isMinorCiv then
							...
							n = g_deal:IsPossibleToTradeItem( playerID, g_activePlayerID, TRADE_ITEM_RESOURCES, resourceID, 1 ) and player:GetNumResourceAvailable( resourceID, false )
						end
						if n and n > 0 then
				...
				tip = L( "TXT_KEY_CITYVIEW_RESOURCE_DEMANDED", resource.IconString .. " " .. resource._Name )
				if #civs > 0 then
					tip = tip.."[NEWLINE]"..L"TXT_KEY_EO_RESOURCES_AVAILBLE"..": "..concat(civs, ", " )
				end
			end
		end
		return BannerToolTip( tip )
	...
	CityIsPuppet = function( city )
		if bnw_mode and g_activePlayer:MayNotAnnex() or city:GetOwner() == g_activePlayerID then
			return BannerToolTip( L"TXT_KEY_CITY_PUPPET" )
		else
			return BannerToolTip( L"TXT_KEY_CITY_PUPPET".."[NEWLINE][NEWLINE]"..L"TXT_KEY_CITY_ANNEX_TT" )
		end
	end,
	CityIsRazing = function( city )
		return BannerToolTip( L( "TXT_KEY_CITY_BURNING", city:GetRazingTurns() ) )
	end,
	CityIsResistance = function( city )
		return BannerToolTip( L( "TXT_KEY_CITY_RESISTANCE", city:GetResistanceTurns() ) )
	end,
	CityIsConnected = function( city )
		...
		elseif cityOwner.GetRouteGoldTimes100 then
			tip = tip .. format(" (%+g[ICON_GOLD])", cityOwner:GetRouteGoldTimes100( city ) / 100 )
		elseif cityOwner.GetCityConnectionRouteGoldTimes100 then
			tip = tip .. format(" (%+g[ICON_GOLD])", cityOwner:GetCityConnectionRouteGoldTimes100( city ) / 100 )
		end
		return BannerToolTip( tip )
	end,
	CityIsBlockaded = function()
		return BannerToolTip( L"TXT_KEY_CITY_BLOCKADED" )
	end,
	CityIsOccupied = function()
		return BannerToolTip( L"TXT_KEY_CITY_OCCUPIED" )
	end,
	CityIsCapital = function( city )
		...
		if city:IsOriginalCapital() then
			if ownerID == originalCityOwnerID then
				if ownerID == g_activePlayerID then
					tip = L( "TXT_KEY_VP_DIPLO_TT_YOU_CONTROL_YOUR_CAPITAL", city:GetNameKey() )
				else
					tip = L( "TXT_KEY_VP_DIPLO_TT_SOMEONE_CONTROLS_THEIR_CAPITAL", Players[ownerID]:GetName(), city:GetNameKey() )
				end
			elseif ownerID == g_activePlayerID then
				tip = L( "TXT_KEY_VP_DIPLO_TT_YOU_CONTROL_OTHER_PLAYER_CAPITAL", city:GetNameKey(), Players[originalCityOwnerID]:GetCivilizationShortDescriptionKey() )
			elseif originalCityOwnerID == g_activePlayerID then
				tip = L( "TXT_KEY_VP_DIPLO_TT_OTHER_PLAYER_CONTROLS_YOUR_CAPITAL", Players[ownerID]:GetName(), city:GetNameKey() )
			else
				tip = L( "TXT_KEY_VP_DIPLO_TT_OTHER_PLAYER_CONTROLS_OTHER_PLAYER_CAPITAL", Players[ownerID]:GetName(), city:GetNameKey(), Players[originalCityOwnerID]:GetCivilizationShortDescriptionKey() )
			end
		end
		return BannerToolTip( tip )
	end,
} -- g_cityToolTips
g_cityToolTips.CityIsOriginalCapital = g_cityToolTips.CityIsCapital

-------------------------------------------------
-- Clear Hex Highlighting
-------------------------------------------------
local function ClearHexHighlights()
	EventsClearHexHighlightStyle( "Culture" )
...
-- Update City Banners
-------------------------------------------------
local RefreshCityBannerEvent = LuaEvents.EUI_UpdateCityBanners.Call
local RefreshCityBanner = function( plotIndex )
	if plotIndex then
		DirtyCityBanners[ plotIndex ] = true
		if IsProcessingDone then
			IsProcessingDone = false
			return RefreshCityBannerEvent()
		end
	end
end
...
local function RefreshCityBannerAtPlot( plot )
	if plot then
		return RefreshCityBanner( plot:GetPlotIndex() )
	end
end
...
local function InitBannerCallbacks( instance )
	...
	for controlID, toolTipCallback in pairs( g_cityToolTips ) do
		local control = instance[ controlID ]
		if control then
			control:SetToolTipType( "EUI_CityBannerTooltip" )
			control:SetToolTipCallback( function() toolTipCallback( GetPlotByIndex( button:GetVoid1() ):GetPlotCity() ) end )
		end
	end
end
...
LuaEvents.EUI_UpdateCityBanners.Add( function()

	local isDebug = Game.IsDebugMode() or g_activePlayer:IsObserver()
	------------------------------
	-- Loop all dirty city banners
	for plotIndex in pairs( DirtyCityBanners ) do
		...
		if city then
			...
			-- not active team city
				...
				if isMinorCiv then
					...
					instance.CityQuests:SetHide( true )
					local civInfo = GameInfo.Civilizations[ cityOwner:GetCivilizationType() ]or{}
			...
			if g_cityHexHighlight == plotIndex then
				...
				if not (civ5_mode and InStrategicView()) then
				...
				if isActiveType then
					...
					for i = 1, #purchasablePlots do
						...
						EventsSerialEventHexHighlight( ToHexFromGrid{ x=plot:GetX(), y=plot:GetY() }, true, g_colorCulture, "Culture" )
					end
...
Events.SerialEventImprovementCreated.Add( function( hexX, hexY, cultureID, continentID, playerID )--, improvementID, rawResourceID, improvementEra, improvementState )
	if playerID == g_activePlayerID then
		...
		if plot then
			...
			if city then
				return RefreshCityBannerAtPlot( city:Plot() )
			end
		end
	end
end)
	...
	for playerID = 0, #Players do
		...
		if player and player:IsAlive() and player:GetTeam() == teamID1 then
			for city in player:Cities() do
				...
				if plot:IsRevealed( g_activeTeamID, true ) then
					RefreshCityBannerAtPlot( plot )
				end
			end
		end
	end
