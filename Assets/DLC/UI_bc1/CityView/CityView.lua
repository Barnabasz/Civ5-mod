...
local g_slackerTexture = civBE_mode and "UnemployedIndicator.dds" or g_slotTexture[ (GameInfo.Specialists[GameDefines.DEFAULT_SPECIALIST] or {}).Type ]
...
local function Pedia( row )
	return Events.SearchForPediaEntry( row and row._Name )
end
...
local function GetSpecialistYields( city, specialist )
	...
	if city and specialist then
		...
		-- Culture
		local cultureFromSpecialist, specialistYield, specialistYieldModifier, yieldInfo = city:GetCultureFromSpecialist( specialistID )
...
local function SpecialistTooltip( control )
	...
	local strToolTip = specialist._Name .. " " .. GetSpecialistYields( GetHeadSelectedCity(), specialist )
...
local function SelectionToolTip( control )
	return OrderItemTooltip( GetHeadSelectedCity(), true, false, control:GetVoid1(), control:GetVoid2() )
end
...
--			local enhancedYieldTechID = GameInfoTypes[ building.EnhancedYieldTech ]
		...
		controls.Label:SetText( concat( tips, " ") )
...
--		controls.Stack:ReprocessAnchoring()
		buildingButton:SetSizeY( max(64, controls.Stack:GetSizeY() + 16) )
	end
...
local g_SelectionListTooltips = {
	...
	GoldButton = function( control )
		return OrderItemTooltip( GetHeadSelectedCity(), control:IsDisabled(), g_yieldCurrency, control:GetVoid1(), control:GetVoid2() )
	end,
	FaithButton = function( control )
		return OrderItemTooltip( GetHeadSelectedCity(), control:IsDisabled(), YieldTypes.YIELD_FAITH, control:GetVoid1(), control:GetVoid2() )
	end,
}
...
local function UpdateWorkingHexesNow()
	...
	if city and UI.IsCityScreenUp() then
		...
		EventsRequestYieldDisplay( YieldDisplayTypesAREA, RadiusHexArea( cityArea ), city:GetX(), city:GetY() )
...
LuaEvents.EUI_UpdateCityScreen.Add( function()
...
local UpdateCityScreenEvent = LuaEvents.EUI_UpdateCityScreen.Call
local function UpdateStuff()
	if IsProcessingDone then
		IsProcessingDone = false
		UpdateCityScreenEvent()
	end
end
...
local function SetToolTipString( toolTipFunc )
	g_leftTipControls.Text:SetText( toolTipFunc( GetHeadSelectedCity() ) )
...
for addin in Modding.GetActivatedModEntryPoints("CityViewUIAddin") do
	...
	if ok then
		...
		print( "Error loading CityViewUIAddin", addinPath, result )
	end
end
