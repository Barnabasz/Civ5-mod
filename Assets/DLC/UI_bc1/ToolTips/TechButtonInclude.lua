...
function GetTechPedia( techID, void2, button )
	...
	if rowType:sub(1,6) == "BUILD_" then
		...
		elseif row.ImprovementType then
			row = GameInfo.Improvements[ row.ImprovementType ]
...
-- todo		row = GameInfo.Technologies[techID]
	...
	elseif civBE_mode and rowType:sub(1,10) == "TERRAFORM_" then
		...
		elseif row.FeatureTypeChange then
			row = GameInfo.Features[ row.FeatureTypeChange ]
		elseif row.TerrainTypeChange then
			row = GameInfo.Terrains[ row.TerrainTypeChange ]
		end
	end
	Events.SearchForPediaEntry( row and row._Name or techPediaSearchStrings[tostring(button)] )
end
...
local function resourceToolTip( techID, void2, button )
	...
	return button:LocalizeAndSetToolTip( "TXT_KEY_REVEALS_RESOURCE_ON_MAP", g_buttonItemInfo[tostring(button)]._Name )
...
local function actionToolTip( techID, void2, button )
	local row = g_buttonItemInfo[tostring(button)]
	local improvementID = row and GameInfoTypes[ row.ImprovementType ]
	if improvementID then
		button:SetToolTipString( GetHelpTextForImprovement( improvementID ) )
	else
		button:SetToolTipString( row._Name )
	end
end
...
local function AdjustArtOnGrantedItem( button, row, textureSize, toolTip, pedia )
	if button then
		...
		button:RegisterCallback( Mouse.eRClick, GetTechPedia )
		return IconHookup( row.IconIndex or row.PortraitIndex or 0, textureSize, row.IconAtlas or "GENERIC_FUNC_ATLAS", button )
	end
end
-- ===========================================================================
...
function AddSmallButtonsToTechButton( thisTechButtonInstance, tech, maxSmallButtons, textureSize, startingButtonNum )
	...
	local function addSmallGenericButtonNZ( a, s )
		if (tonumber(a) or 0) ~= 0 then
			return addSmallButtonAndPedia( tech, 0, "GENERIC_FUNC_ATLAS", s, a )
		end
	end
	...
	if civBE_mode then
		...
		for tech in GameInfo.Technology_Affinities( thisTechType ) do
			...
			if affinityInfo then
				local button = addSmallButtonAndPedia( GameInfo.Concepts[ (GameInfo.Affinity_Types[ tech.AffinityType ] or {}).CivilopediaConcept ], unpack( affinityInfo ) )
				if button then
	...
	for row in GameInfo.Processes( thisTechPrereq ) do
		if not addSmallButton( row.PortraitIndex, row.IconAtlas, "TXT_KEY_ENABLE_PRODUCITON_CONVERSION", row._Name ) then
			break
		end
	end
	...
	if civBE_mode then
		for row in GameInfo.Technology_FreePlayerPerks( thisTechType ) do
			if not addSmallArtButton( AdjustArtOnGrantedPlayerPerkButton, GameInfo.PlayerPerks[row.PlayerPerkType] ) then
				break
			end
		end
	end
	...
	for row in GameInfo.Route_TechMovementChanges( thisTechType ) do
		if not addSmallGenericButton( "TXT_KEY_FASTER_MOVEMENT", (GameInfo.Routes[row.RouteType] or {})._Name ) then
			break
		end
	end
	...
	for row in GameInfo.Improvement_TechYieldChanges( thisTechType ) do
		...
		if improvement and validImprovementBuilds[improvement.Type] then
			...
			for row in GameInfo.Improvement_TechYieldChanges( thisTechAndImprovementTypes ) do
				if (row.Yield or 0)~=0 then
					icon = (GameInfo.Yields[row.YieldType]or{}).IconString or "?"
					icons = icons .. icon
					toolTip = ("%s +%g%s"):format( toolTip, row.Yield, icon )
				end
			end
			if not addSmallActionButton( GameInfo.Builds{ ImprovementType = improvement.Type }(), icons, toolTip ) then
				break
			end
		end
	end
	...
	for row in GameInfo.Improvement_TechNoFreshWaterYieldChanges( thisTechType ) do
		...
		if yield and not addSmallActionButton( GameInfo.Builds{ ImprovementType = row.ImprovementType }(), yield.IconString,
				"TXT_KEY_NO_FRESH_WATER", (GameInfo.Improvements[row.ImprovementType] or {})._Name or "???" , yield._Name, row.Yield )
		then
			break
		end
	end
	...
	for row in GameInfo.Improvement_TechFreshWaterYieldChanges( thisTechType ) do
		local yield = GameInfo.Yields[row.YieldType]
		if yield and not addSmallActionButton( GameInfo.Builds{ ImprovementType = row.ImprovementType }(), yield.IconString,
				"TXT_KEY_FRESH_WATER", (GameInfo.Improvements[row.ImprovementType] or {})._Name, yield._Name, row.Yield )
		then
			break
		end
	end
	...
	for row in GameInfo.Buildings{ EnhancedYieldTech = techType } do
		if validBuildingBuilds[row.BuildingClass] == row.Type then
			...
			if not addSmallActionButton( row, icons, toolTip ) then
				break
			end
		end
	end
	...
	if gk_mode then
		addSmallGenericButtonNZ( tech.UnitFortificationModifier, "TXT_KEY_UNIT_FORTIFICATION_MOD" )
		addSmallGenericButtonNZ( tech.UnitBaseHealModifier, "TXT_KEY_UNIT_BASE_HEAL_MOD" )
	...
	if bnw_mode then
		...
		for row in GameInfo.Technology_TradeRouteDomainExtraRange( thisTechType ) do
			if row.TechType == techType and (row.Range or 0) > 0 then
				local toolTip = ""
				local domainID = (GameInfo.Domains[ row.DomainType ] or {}).ID
		...
		if civ5_mode then
			addSmallGenericButtonNZ( tech.InfluenceSpreadModifier, "TXT_KEY_DOUBLE_TOURISM" )
			addSmallGenericButtonIF( tech.AllowsWorldCongress and "TXT_KEY_ALLOWS_WORLD_CONGRESS" )
			addSmallGenericButtonNZ( tech.ExtraVotesPerDiplomat, "TXT_KEY_EXTRA_VOTES_FROM_DIPLOMATS" )
