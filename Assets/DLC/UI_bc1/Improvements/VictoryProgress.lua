...
local g_DominationRowsIM = GenerationalInstanceManager:new( "DominationRow", "RowStack", Controls.DominationStack );
...
local g_TechIM = GenerationalInstanceManager:new( "TechCiv", "Civ", Controls.TechStack );
local g_TechList = {};

-- Diplo Details Variables
local g_DiploIM = GenerationalInstanceManager:new( "DiploCiv", "Civ", Controls.DiploStack );
local g_DiploList = {};

-- Cultural Details Variables
local g_CultureRowsIM = GenerationalInstanceManager:new( "CultureRow", "RowStack", Controls.CultureStack );
...
local g_ScoreIM = GenerationalInstanceManager:new( "ScoreCiv", "Civ", Controls.ScoreStack );
local g_ScoreList = {};
		...
		for iTeamLoop = 0, GameDefines.MAX_CIV_TEAMS-1, 1 do
			...
			for iPlayerLoop = 0, GameDefines.MAX_MAJOR_CIVS-1, 1 do
				...
				if not pPlayer:IsMinorCiv() and pPlayer:IsEverAlive() and pPlayer:GetTeam() == iTeamLoop then
					if pPlayer:IsHasLostCapital() or not pPlayer:IsAlive() then --bc1 fix
		...
		for iPlayerLoop = 0, GameDefines.MAX_CIV_PLAYERS-1, 1 do
			...
			if not pPlayer:IsMinorCiv() and pPlayer:IsEverAlive() then
				...
				if curRow == nil then
					...
					curItemIM = GenerationalInstanceManager:new( "DominationItem", "IconFrame", curRow.RowStack );
...
function PopulateDiplomatic()
	...
	if victoryId and PreGame.IsVictory(victoryId) then
		...
		if Game.GetVictory() == victoryId then
			...
			if Game.GetNumActiveLeagues() > 0 then
				...
				if pLeague and Game.IsUnitedNationsActive() then
		...
		for iPlayerLoop = 0, GameDefines.MAX_CIV_PLAYERS-1, 1 do
			...
			if iPlayerLoop ~= iActivePlayer and not pPlayer:IsMinorCiv() and pPlayer:IsEverAlive() then
				...
				if curRow == nil then
					...
					numItemIM = numItemIM + 1;
					curItemIM = GenerationalInstanceManager:new( "CultureItem", "IconFrame", curRow.RowStack );
...
function SetCivIcon(pPlayer, iconSize, controlTable, controlTableTT, controlLost, lostCondition, controlBG, controlShadow)
	...
	if lostCondition and controlLost then
