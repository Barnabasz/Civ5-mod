...
--		local themCivInfo = GameInfo.Civilizations[ g_pThem:GetCivilizationType() ]
	...
	SetEnabledAndTextAndToolTip( Controls.UsPocketGold, isEnabled,
		...
		bnw_mode and not isEnabled and "[COLOR_WARNING_TEXT]"..L"TXT_KEY_DIPLO_NEED_DOF_TT_ONE_LINE".."[ENDCOLOR]" )
...
--		toolTip = toolTip .. L"TXT_KEY_DIPLO_OPEN_BORDERS_HAVE":gsub("%[COLOR_NEGATIVE_TEXT%]","[COLOR_POSITIVE_TEXT]")
	if not g_pThemTeam:IsOpenBordersTradingAllowed() then
		toolTip = toolTip .. "[COLOR_WARNING_TEXT]" .. L(gk_mode and "TXT_KEY_DIPLO_OPEN_BORDERS_NO_TECH" or "TXT_KEY_DIPLO_NEED_WRITING_TT") .. "[ENDCOLOR]"
...
--		toolTip = toolTip .. "[COLOR_POSITIVE_TEXT]" .. L"TXT_KEY_DIPLO_DEF_PACT_EXISTS" .. "[ENDCOLOR]"
	if not g_pUsTeam:IsDefensivePactTradingAllowed() and not g_pThemTeam:IsDefensivePactTradingAllowed() then
		toolTip = toolTip .. L"TXT_KEY_DIPLO_DEF_PACT_NO_TECH"
...
--		toolTip = L"TXT_KEY_DIPLO_TRADE_AGREEMENT_NO_AGREEMENT"
	...
	for resourceID, instance in pairs( g_UsPocketResources ) do
		...
		if g_Deal:IsPossibleToTradeItem( g_iUs, g_iThem, TradeableItems.TRADE_ITEM_RESOURCES, resourceID, 1 ) then	-- 1 here is 1 quanity of the Resource, which is the minimum possible
			...
			instance.Button:SetText( resource.IconString .. resource._Name .. " (" .. ( gk_mode and g_Deal:GetNumResource(g_iUs, resourceID) or g_pUs:GetNumResourceAvailable( resourceID, false ) ) .. ")" )
		else
	...
	for resourceID, instance in pairs( g_ThemPocketResources ) do
		if g_Deal:IsPossibleToTradeItem(g_iThem, g_iUs, TradeableItems.TRADE_ITEM_RESOURCES, resourceID, 1) then -- 1 here is 1 quanity of the Resource, which is the minimum possible
			...
			instance.Button:SetText( resource.IconString .. " " .. resource._Name .. " (" .. (gk_mode and g_Deal:GetNumResource(g_iThem, resourceID) or g_pThem:GetNumResourceAvailable( resourceID, false )) .. ")" )
		else
			instance.Button:SetHide( true )
		end
	end
...
do
	local function AddGoldTrade( control, playerID, gold )
		g_Deal:AddGoldTrade( playerID, math_min( UI.ShiftKeyDown() and math_huge or gold, g_Deal:GetGoldAvailable( playerID, -1 ) ) ) -- This is -1 because we're not changing anything right now
		DoUIDealChangedByHuman()
		control:TakeFocus()
	end
	...
	local function ChangeGoldTrade( string, control, playerID, parentControl )
		local gold = math_min( tonumber( string ) or 0, g_Deal:GetGoldAvailable( playerID, TradeableItems.TRADE_ITEM_GOLD ) )
...
do
	local function AddGoldPerTurnTrade( control, playerID, player, goldPerTurn )
		g_Deal:AddGoldPerTurnTrade( playerID, math_min( UI.ShiftKeyDown() and math_huge or goldPerTurn, player:CalculateGoldRate() ), g_iDealDuration )
		DoUIDealChangedByHuman()
		control:TakeFocus()
	end
...
do
	...
	local function AddResourceInstance( resource, instanceName, stack, list, clickAction, changeAction )
		...
		instance.Button:SetText( resource.IconString .. resource._Name )
