...
Timer:RegisterAnimCallback( function()
	g_toolTipControls.Stack:SetWrapWidth( 32 )
	for _, instance in pairs ( g_usedPromotions ) do
		instance.Text:SetHide( false )
	end
	g_toolTipControls.Stack:CalculateSize()
			...
			for unitPromotion in GameInfo.UnitPromotions() do
				if unit:IsHasPromotion(unitPromotion.ID) and unitPromotion.ShowInUnitPanel ~= false then
					...
					if not instance then
						instance = remove( g_sparePromotions )
						if instance then
					...
					instance.Text:SetText( unitPromotion._Name )
					instance.Text:SetHide( true )
				end
			end
		end
		for j = i+1, #g_usedPromotions do
			local instance = remove( g_usedPromotions )
			insert( g_sparePromotions, instance )
			instance.Promotion:ChangeParent( g_ScrapControls )
		end
		g_toolTipControls.Stack:SetWrapWidth( ceil( i / ceil( i / 10 ) ) * 26 )
		g_toolTipControls.Stack:CalculateSize()
		g_toolTipControls.Box:DoAutoSize()
...
function( playerID, unitID, isDimmed )
	...
	local flag = g_UnitFlags[ playerID ][ unitID ]
	if flag then
		return flag.FlagShadow:SetAlpha( isDimmed and 0.5 or 1.0 )
	else
