...
include("HBTMapGenerator");
include("HBTMultilayeredFractal");
include("HBTFeatureGenerator");
include("HBTTerrainGenerator");

------------------------------------------------------------------------------
function GetMapScriptInfo()
	local world_age, temperature, rainfall, sea_level, resources = GetCoreMapOptions()
	return {
		Name = "HellBlazers Teamer Map v3",
		Description = "HellBazers Teamer Map",
...
function MultilayeredFractal:GeneratePlotsByRegion()
	...
	local fracFlags = {};
	local SplitOps = Map.GetCustomOption(6);
...
function AssignStartingPlots:GenerateRegions(args)
	...
	local args = args or {};
	local iW, iH = Map.GetGridSize();
	local splitsel = Map.GetCustomOption(6)
	local coastallow = Map.GetCustomOption(7)
	local res = Map.GetCustomOption(4)
	if res == 9 then
		res = 1 + Map.Rand(3, "Random Resources Option - Lua");
	end

	print("Center Option: ", splitsel);
	print("Coast Starts: ", coastallow);

	local setback = 2;

	if coastallow == 2 then
		setback = 4;
	end
	...
	if iNumTeams == 2 and team_setting == 1 then
		...
		self.inhabited_Width = (math.floor(iW / 2)) - setback;
...
function SetDivide()
	...
	elseif SplitOps == 3 then
		...
		-- Add strip of jungle to middle of map
		for y = 0, iH - 1 do
			for x = math.floor(iW / 2) - 2, math.floor(iW / 2) + 1 do
				local plot = Map.GetPlot(x, y)
				if plot:GetPlotType() == PlotTypes.PLOT_MOUNTAIN then
	...
	elseif SplitOps == 4 then
		...
		-- Add strip of forest to middle of map
		for y = 0, iH - 1 do
			for x = math.floor(iW / 2) - 2, math.floor(iW / 2) + 1 do
				local plot = Map.GetPlot(x, y)
				if plot:GetPlotType() == PlotTypes.PLOT_MOUNTAIN then
	...
	elseif SplitOps == 6 then
		--Desert

		-- Add strip of desert to middle of map
		for y = 0, iH - 1 do
			for x = math.floor(iW / 2) - 2, math.floor(iW / 2) + 1 do
				local plot = Map.GetPlot(x, y)
				if plot:GetPlotType() == PlotTypes.PLOT_MOUNTAIN then
